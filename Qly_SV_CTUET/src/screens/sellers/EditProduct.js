...
const EditProduct = ({ navigation, route }) => {
  ...
  const [qty, setQty] = useState({ value: item.soluong.toString(), error: "" });
  const [productName, setProductName] = useState({
    value: item.tensp,
    error: "",
  });
  const [price, setPrice] = useState({ value: item.gia.toString(), error: "" });
  ...
  const onPressThemSP = async () => {
    ...
    if (nameErr || qtyErr || pricerErr) {
      ...
      if (image != "") {
        ...
        const fileRef = ref(getStorage(), "image/sanpham/" + item.idsp);
        const result2 = await uploadBytes(fileRef, blob);

        getDownloadURL(fileRef).then((url) => {
          updateDoc(doc(db, "sanpham", item.idsp), {
    ...
    <>
      ...
      >
        ...
        >
          ...
          <Image
            source={{
              uri: image != item.image ? image : item.image,
            }}
        ...
        <SelectDropdown
          data={directoryData}
          defaultValue={item.idDanhMuc}
          onSelect={(selectedItem, index) => {
            setIdDirectory(selectedItem.idDanhMuc);
          }}
          defaultButtonText={
            item.idDanhMuc != "" ? item.TenDanhMuc : "Danh Mục"
          }
